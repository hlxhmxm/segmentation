...
import torch.nn.functional as F
from mmseg.registry import MODELS
from mmseg.models.backbones.twins import PCPVT


@MODELS.register_module()
class TwinsPCPVTWrapper(nn.Module):
    def __init__(self,
                 out_indices=(0, 1, 2),
                 init_cfg=None):
        super().__init__()
        ...
        # Adaptar canales a los esperados por BiSeNet
        self.conv_4 = nn.Conv2d(64, 64, kernel_size=1)     # 1/4
        self.conv_8 = nn.Conv2d(128, 64, kernel_size=1)    # 1/8
        self.conv_16 = nn.Conv2d(256, 128, kernel_size=1)  # 1/16
        self.conv_32 = nn.Conv2d(128, 256, kernel_size=1)  # dummy 1/32

    def forward(self, x):
        outs = self.backbone(x)  # outs: [x_4, x_8, x_16]

        # Resize para entregar 4 salidas como espera BiSeNetV1
        out_4 = self.conv_4(F.interpolate(outs[0], scale_factor=2, mode='bilinear', align_corners=False))  # 1/4
        out_8 = self.conv_8(outs[1])    # 1/8
        out_16 = self.conv_16(outs[2])  # 1/16

        # Generar dummy x_32 para cumplir con el unpacking
        out_32 = self.conv_32(F.adaptive_avg_pool2d(out_16, (out_16.shape[2] // 2, out_16.shape[3] // 2)))  # 1/32

        return [out_4, out_8, out_16, out_32]
